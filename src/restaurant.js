...
const createMenu = (myMenu) => { 
  let objeto = {
    fetchMenu: () => myMenu, 
    consumption: [],
    order: (request) => objeto.consumption.push(request),
    pay: () => {
      let valorTotal = 0;
      let listaDeValoresObj = Object.assign(objeto.fetchMenu().drink, objeto.fetchMenu().food);
      let listaDeValores = Object.entries(listaDeValoresObj);
      objeto.consumption.forEach((i) => { 
        valorTotal += listaDeValores.find((o) => o[0] === i)[1]; 
      });
